...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingResponseDto> getAll(Long userId, State state, Integer from, Integer size) {
        return getBookingByStateForBooker(userId, state, from, size);
    }

    @Override
    public List<BookingResponseDto> getOwnerBookings(Long userId, State state, Integer from, Integer size) {
        return getBookingsByStateForOwner(userId, state, from, size);
    }

    private List<BookingResponseDto> getBookingByStateForBooker(Long userId, State state, Integer from, Integer size) {
    ...
    private List<BookingResponseDto> getBookingsByStateForOwner(Long userId, State state, Integer from, Integer size) {
        findUserOrThrow(userId);
        ...
        List<Booking> bookings = switch (state) {
            ...
            case WAITING -> bookingRepository.findByItemOwnerIdAndStatus(userId, Status.WAITING, pageable);
            case REJECTED -> bookingRepository.findByItemOwnerIdAndStatus(userId, Status.REJECTED, pageable);
        };
        return bookings.stream().map(BookingMapper::toDto).toList();
    }
